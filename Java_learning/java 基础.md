...
![image-20230209153244430](C:\Users\29844\AppData\Roaming\Typora\typora-user-images\image-20230209153244430.png)
...
**两个浮点类型的包装类（Float，Double**）**没有实现缓存**

```java
Integer i1 = 33;
Integer i2 = 33;
System.out.println(i1 == i2);// 输出 true 说明有缓存的存在
...
因为数据的大小超过一定的范围不会使用包装类的缓存，所以不能直接用==来进行比较，但是可以使用equals方法进行判断
...
**字符串常量池** 是 JVM 为了提升性能和减少内存消耗针对字符串（String 类）
...
ArrayList的底层是数组，相当于一个动态的数组。和普通数组相比，能够动态的增长，增加容量
