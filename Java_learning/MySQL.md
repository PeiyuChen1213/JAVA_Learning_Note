...
MVVC的实现依赖于read view 和undo log 还有就是索引当中的隐藏字段

**read view 当中的重要字段**：

1. creator_trx_id: 创建read view 的事务id
2. m_ids ：当前活跃的事务列表
3. max_id：当前数据库当中下一个事务的事务id
4. min_id：当前活跃当中最小的id
...
2. roll_pointer: 每次进行改动的时候，就会将旧版本记录到undo log 当中 新版本和旧版本之间由指针相连

 在创建read view之后 可以把当前数据库的事务划分为以下几个种类：

![img](./MySQL.assets/ReadView.drawio.png)

可见性关系的总结：

1. 如果记录的事务id 小于min_trx_id 说明该记录是在创建read view 已经提交的事务产生的，对当前的事务是可见的
2. 相反，如果大于max_trx_id 则说明该记录是创建当前当前read view 之后生成的 所以是不可见的
3. 如果该记录的事务id 介于上述两种情况之间，则需要进行判断。修改该记录的事务是否已经提交 （在不在m_ids）如果不在 就是未提交也是不可见的 如果提交了是可见的
...
假设事务 A （事务 id 为51）启动后，紧接着事务 B （事务 id 为52）也启动了
...
1. 当事务B去读取这条记录的时候，发现记录的事务id小于 说明 这个记录是在创建之前已经提交了是可见的 所以读到100万
2. 事务A去修改之后，记录的事务id就会修改为51，并且undo log会记录修改之前的数据信息 ，然年用指针串联起来
3. 事务再去读取记录的时候发现该记录的事务id介于最大和最小之间，并且在m_ids集合里面 说明当前记录还是未提交的，所以不可见，找到前一条记录
4. A事务提交之后 但是在可重复读的情况下只会创建一次read view  所以read view不变 事务B再次查看的记录的时候和步骤3一致还是只能查到原来的记录
...
*在读已提交的隔离级别下每次读取数据的时候都会创建一个**read view***
...
1. 事务B 读取事务的时候，创建 read view 数据的事务id小于事务B的id 是可见的，所以读取到100万
2. 当事务A修改数据的时候 数据的事务id变成 51 
